...
macro_rules! impl_write_to_buf {
    ($type:ident, $arg_type:expr) => {
        impl WriteToBuf for $type {
            fn write(&self, mut buf: &mut [u8]) -> Result<usize, ()> {
                TagLenValue::<ArgType>::new($arg_type, &self.to_ne_bytes()).try_write(&mut buf)
            }
        }
    };
}
...
impl WriteToBuf for str {
    fn write(&self, mut buf: &mut [u8]) -> Result<usize, ()> {
        TagLenValue::<ArgType>::new(ArgType::Str, &self.as_bytes()).try_write(&mut buf)
    }
}
...
#[doc(hidden)]
pub fn write_record_message(mut buf: &mut [u8], msg: &str) -> Result<usize, ()> {
    TagLenValue::<RecordField>::new(RecordField::Log, msg.as_bytes()).try_write(&mut buf)
}
